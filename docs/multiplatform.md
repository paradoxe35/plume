...
The compiler also missed one, which is the more interesting half. The dictionary _values_ were
...
**The window and the taskbar** read `java.awt.Window.iconImages`, a _set_. Handing over a single
...
is too early to know what the user meant: ctrl+cmd+space opens the emoji picker, and ctrl+cmd is a
prefix of it. Every reach for the picker revised the selection on the way past.

Nothing about the modifiers themselves distinguishes the two — the difference is what happens next.
So a modifier-only binding now fires on **release**, and only when nothing else happened while the
combination was held: no other key, no extra modifier, no click. The chord is remembered from the
first modifier down to the last one up, and the first modifier to come up ends it, so releasing the
rest cannot fire it a second time.

Bindings that name a key keep firing on the key press, where there is nothing to disambiguate.

Two things fell out of writing it as a state machine rather than a matcher over strings. Auto-repeat
is no longer a second press, so holding a shortcut runs the action once instead of flooding the tray
with "already working". And the binding's modifiers are resolved once at registration rather than by
re-scanning a `Vec<String>` on every key press — which mattered more than it looks, since that ran
inside the system's own event callback.
...
controller took that as what the _listener_ should do. The two readings are opposites.
...
the comment _"the insets are queried synchronously and are not cached"_ — it hops to the AppKit
...
_bounds_ now, which need no such round trip; the cost is ignoring the menu bar and the Dock, worth
a few pixels.
...
therefore _raises an `NSException`_ for any pending Java exception with nothing to catch it, having
cleared the Java exception first. The JDK header says as much: _"control will propagate back to the
run loop which might terminate the application… the location of termination does not show where the
NSException originated."_ That is the whole crash report, explained. Plume asks for Metal at
